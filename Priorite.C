...
void executerPRIO(Processus processus[], int n,Processus processus2[]) {
    int temps_actuel = 0;
    ...
    while (processus_termine < n)
     {
        int indice = -1;
        int prio_max = -8989898;

        for (int i = 0; i < n; i++)
          {
            if (processus[i].date_arrivee <= temps_actuel && processus[i].priorite > prio_max && processus[i].duree_cycles > 0) {
                indice = i;
        ...
        ajoute_exe_proc(tableau, &nombre, processus[indice].nom_processus,temps_actuel,temps_actuel+1);  
        processus[indice].duree_cycles--;
        temps_actuel++;

        if (processus[indice].duree_cycles == 0) 
          {
            processus_termine++;         
        temps_rotation_p = temps_actuel - processus[indice].date_arrivee ;
        int duree_cycle_p= recuperer_duree(processus[indice].nom_processus,processus2,n);     			   		   
        temps_attente_p = temps_rotation_p - duree_cycle_p;    			   
        printf("temps de rotation de (%s) = %d 	 temps d attente de (%s) = %d \n ",processus[indice].nom_processus,temps_rotation_p,processus[indice].nom_processus,temps_attente_p );
...
int main(int argc, char *argv[]) {
    ...
    executerPRIO(processus,n,copieProcessus);
    return 0;
}
